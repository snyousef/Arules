#Package for analyzing transactional data
install.packages("arules")
library(arules)
require(arules) #loads all techniques within arules into environment
??arules

#Package that provides visual techniques for the arules package; required to install a few extra packages (TSP, caTools, whisker, and fpc) to properly run arulesViz
...
#Upload transaction comma-separated data
Trans <- read.transactions("ElectronidexTransactions2017.csv", sep = ",") #Changes data into a sparse matrix (i.e.: each different product will have its own column and a '1' in the transaction row if the product was purchased (observation) and a '0' if it wasn't purchased)
Trans #Returns all data from csv file; Question: Should I remove duplicate items? How do I remove items that meet certain requirements (i.e.: transactions with zero items purchased)

summary(Trans)
#Function above returns basic information about transaction data
...
#Question: Why are there two transactions with zero items purchased
...
write.table(itemLabels(Trans), "ItemsforSale.txt") #Lists all the products sold and puts them into a text file
...
image(Trans[1:100]) #x-axis numbers correspond to each item sold; y-axis correspond to the transaction number; each dot corresponds to whether that item 'x' was purchased in transaction 'y'
image(sample(Trans, 100)) #Does the same thing as above but takes a random sample of whatever amount specified



#Confidence is a measure of the propotion of transactions where the presence of an item or set of items results in the presence of another set of items
...
summary(MBRule) #Prints out general information about rules foundl; lhs means left-hand side, and rhs means right-hand side
...
#In printout results below, Apple Macbook Air is 2.48 times more likely to be purchased when a Dell KM117 Wireless Keyboard & Mouse are purchased than an Apple Macbook Air is to purchased among all transactions
...
WriteTable <- write.table(MBRuleInspect, "MBRuleInspect.txt")
